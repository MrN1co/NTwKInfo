...
**TU UZUPEŁNIĆ:** 2–5 zdań: co robi moduł i jaka jest jego odpowiedzialność.
...
Przykład:
- **US-123** — Jako użytkownik chcę zobaczyć aktualną pogodę dla 5 lokalizacji.
- **US-124** — Jako użytkownik chcę zobaczyć alert o temperaturze poniżej 0°C.
...
- `modules/ekonomia/klasy_api_obsluga/` — warstwa serwisowa:
  ...
  - `HistoricalData.py` — zbieranie historycznych danych (dzielenie zakresów ze względu na limity API) i konwersja do DataFrame.
...
|---:|---|---|---|---|---|
| GET | /weather | HTML | Widok bieżącej pogody | US-201 | api_reference.md#weather-html |
| GET | /api/weather/forecast | JSON | Dane pogodowe do UI | US-201 | api_reference.md#weather-forecast |
| POST | /api/weather/favorites | JSON | Zapis ulubionych miast | US-202 | api_reference.md#weather-favorites |
