...
def save_to_json(filepath, data):
    ...
    print(f"[{get_warsaw_time()}] Zapisano dane do {filepath}")
...
def run_initial_scrape():
    ...
    print(f"[{get_warsaw_time()}] Rozpoczynam jednorazowe scrapowanie przy starcie aplikacji")
...
def atp_daemon(interval):
    ...
    """
    print(f"[{get_warsaw_time()}] Daemon ATP uruchomiony (interwał: {interval}s)")
    
    while True:
        print(f"[{get_warsaw_time()}] Scrapowanie ATP...")
        success = update_tennis_rankings('atp', get_atp_rankings)
        if success:
            print(f"[{get_warsaw_time()}] Aktualizacja ATP zakończona powodzeniem")
        else:
...
def wta_daemon(interval):
    ...
    Args:
        interval: interwał w sekundach między scrapowaniem
    """
    print(f"[{get_warsaw_time()}] Daemon WTA uruchomiony (interwał: {interval}s)")
    
    while True:
        print(f"[{get_warsaw_time()}] Scrapowanie WTA...")
        success = update_tennis_rankings('wta', get_wta_rankings)
        if success:
            print(f"[{get_warsaw_time()}] Aktualizacja WTA zakończona powodzeniem")
        else:
...
def polish_leagues_daemon():
    ...
    """
    print(f"[{get_warsaw_time()}] Daemon Polish Leagues uruchomiony")
    ...
    print(f"[{get_warsaw_time()}] Znaleziono {len(leagues)} polskich lig do scrapowania")
    print(f"[{get_warsaw_time()}] Interwał: {interval}s")
    ...
    while True:
        try:
            ...
            print(f"[{get_warsaw_time()}] Scrapowanie {league_name}...")

            success = update_polish_league(league)
            if success:
                print(f"[{get_warsaw_time()}] Zaktualizowano {league_name}")
            else:
            ...
            print(f"[{get_warsaw_time()}] Błąd Polish Leagues daemon: {e}")
            time.sleep(10)
...
def football_leagues_daemon():
    ...
    """
    print(f"[{get_warsaw_time()}] Daemon Football Leagues uruchomiony")

    # 10 najważniejszych lig europejskich
    LEAGUES = [
    ...
    print(f"[{get_warsaw_time()}] Scrapowanie {len(LEAGUES)} lig rotacyjnie")
    print(f"[{get_warsaw_time()}] Interwał: {interval}s (każda liga co {interval * len(LEAGUES)}s)")
    ...
    while True:
        try:
            ...
            print(f"[{get_warsaw_time()}] Scrapowanie {league_code}...")

            success = update_football_league(league_code)
            if success:
                print(f"[{get_warsaw_time()}] Zaktualizowano {league_code}")
            else:
...
def nba_daemon(interval):
    ...
    Args:
        interval: interwał w sekundach między scrapowaniem
    """
    print(f"[{get_warsaw_time()}] Daemon NBA uruchomiony (interwał: {interval}s)")

    nba_path = os.path.join(BASE_DIR, 'data', 'news', 'ESPN-API', 'nba_standings.json')
    
    while True:
        print(f"[{get_warsaw_time()}] Scrapowanie NBA...")
        success = update_espn_league('NBA', get_nba_standings, nba_path)
        if success:
            print(f"[{get_warsaw_time()}] Zaktualizowano dane NBA")
        else:
...
def mls_daemon(interval):
    ...
    Args:
        interval: interwał w sekundach między scrapowaniem
    """
    print(f"[{get_warsaw_time()}] Daemon MLS uruchomiony (interwał: {interval}s)")

    mls_path = os.path.join(BASE_DIR, 'data', 'news', 'ESPN-API', 'mls_standings.json')
    
    while True:
        print(f"[{get_warsaw_time()}] Scrapowanie MLS...")
        success = update_espn_league('MLS', get_mls_standings, mls_path)
        if success:
            print(f"[{get_warsaw_time()}] Zaktualizowano dane MLS")
        else:
...
def kryminalki_news_daemon(interval):
    ...
    Args:
        interval: interwał w sekundach między scrapowaniem
    """
    print(f"[{get_warsaw_time()}] Daemon Kryminalki News uruchomiony (interwał: {interval}s)")
    
    news_path = os.path.join(BASE_DIR, 'data', 'news', 'kryminalki.json')
    
    while True:
        print(f"[{get_warsaw_time()}] Scrapowanie wiadomości z Kryminalki.pl...")
        ...
        try:
            ...
            if news_list:
                ...
                print(f"[{get_warsaw_time()}] Zaktualizowano {len(news_list)} wiadomości z Kryminalki.pl")
            else:
...
def start_all_daemons():
    ...
    for thread in threads:
        thread.start()
        print(f"✓ Uruchomiono {thread.name}")

    print("=" * 60)
    print("Wszystkie daemony działają w tle!")
    print("=" * 60)
